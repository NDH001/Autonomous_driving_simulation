...
class Node():
    ...
    def __eq__(self, other):
        return int(floor(self.pos[0])) == int(floor(other.pos[0])) and int(floor(self.pos[1])) == int(floor(other.pos[1]))

    def __lt__(self, other):
        if self.cost_f == other.cost_f:
            return self.cost_h < other.cost_h
        else:
...
class Planner:
    ...
    def get_route(self, node):
        ...
        current_node = node
        while current_node:
            temp_path.append(current_node.pos)
            current_node = current_node.par
        path = temp_path[::-1]
        return path

    def hybrid_algorithm(self, pos_start):

        node_start = Node(None, pos_start)

        visited_list = []
        node_open = [node_start]
        heapq.heapify(node_open)

        # Loop until the end
        while True:
            # pop smallest cost node from candidate and push to the visited list
            current_node = heapq.heappop(node_open)

            visited_list.append(current_node)

            # if it is goal node
            if self._check_goal(current_node.pos):
                print("Reach goal")
            ...
            x, y, theta = current_node.pos


            temp_theta = (-1,-0.5,0,0.5,1)

            for i in temp_theta:

                w = i * np.pi
                position_next = self.motion_predict(x, y, theta, 1, w)
                if (position_next):
                    # Create new node
                    node_next = Node(current_node, position_next)
                    child_nodes.append(node_next)

            for child_node in child_nodes:
                # update cost

                child_node.cost_g = current_node.cost_g + 1

                child_node.cost_h = self._d_from_goal(current_node.pos)

                child_node.cost_f = child_node.cost_g + child_node.cost_h


                #if node is not lowest g cost then no need to add it
                cost_more =[]


                for node in node_open:
                    if child_node == node and child_node.cost_g > node.cost_g:
                        cost_more.append(child_node)

                if len(cost_more) > 0:
                    continue

                node_open.append(child_node)
    ...
    def map_callback(self):
        ...
        for i in range(len(temp[0])):
            ...
            for w in range(inflate):
                ...
                self.aug_map[
                    min(self.world_height - 1, row_index + w), min(self.world_width - 1, column_index + w)] = 100
    ...
    def generate_plan(self):
        ...
        optimal_path = self.hybrid_algorithm(start)

        temp_actions = []

        for i in range(1, len(optimal_path)):
            w = (optimal_path[i][2] - optimal_path[i - 1][2]) * 2
            v = 1
            temp_actions.append((v, w))


        self.action_seq = temp_actions
        print("done")
    ...
    def collision_checker(self, x, y):
        ...
        if (int(x * (self.grid_resolution)) < 2 or int(y * (self.grid_resolution)) < 2):

            return True

        if (int(y * (self.grid_resolution)) > self.world_height - 1 or int(
                x * (self.grid_resolution)) > self.world_width - 1):

            return True


        return self.aug_map[int(y * (self.grid_resolution)), int(x * (self.grid_resolution))] == 100
...
if __name__ == "__main__":
    ...
    inflation_ratio = (int(robot_radius * 10) + 1) * 2 + 7
    ...
    np.savetxt("CSDA_com1.jpg_{}_{}.txt".format(goal[0],goal[1]), result, fmt="%.2e")
